...
func TestReleaseHistoryAdoptionV2BridgesHeadEMAOnceWithoutReset(t *testing.T) {
	...
	if _, _, err := store.FoldForEpochV2(t.Context(), 1405, map[FleetScoreKey]*big.Rat{key: big.NewRat(4, 1)}, alpha); err != nil {
		t.Fatal(err)
	}
	...
	for _, epoch := range []uint64{1407, 1409, 1411} {
		if _, _, err := store.PreviewForEpochV2(t.Context(), epoch, raw, alpha); err == nil {
			t.Fatalf("unapproved bridge epoch %d passed", epoch)
		}
	}
	got, records, err := store.PreviewForEpochV2(t.Context(), 1410, raw, alpha)
	if err != nil || got[7].Cmp(big.NewRat(3, 1)) != 0 {
		t.Fatalf("one real EMA fold differs: %v %v", got, err)
	}
	compact, compactRecords, err := store.previewForEpochV2(t.Context(), 1410, raw, alpha, limits.MaxEntries, limits.MaxControlBytes)
	if err != nil || !reflect.DeepEqual(got, compact) || !equalHeadEMAFolds(records, compactRecords) {
		t.Fatalf("compact bridge disagrees: %v", err)
	}
	after, err := os.ReadFile(store.path)
	if err != nil || !bytes.Equal(before, after) {
		t.Fatalf("bridge preview changed durable EMA: %v", err)
	}
	if err := store.CommitForEpochV2(t.Context(), 1410, records, alpha); err != nil {
		t.Fatal(err)
	}
	...
	if restarted.allowsHeadEMAEpochGaps() || restarted.allowsHeadEMAEpochGapTo(1412) {
		t.Fatal("restart widened the consumed bridge")
	}
	if _, _, err := restarted.PreviewForEpochV2(t.Context(), 1412, raw, alpha); err == nil {
		t.Fatal("future skipped epoch passed after adoption")
	}
	if _, _, err := restarted.PreviewForEpochV2(t.Context(), 1411, raw, alpha); err != nil {
		t.Fatalf("strict consecutive successor failed: %v", err)
	}
}
...
func TestReleaseHistoryAdoptionV2ReplaysEveryInteriorTerminal(t *testing.T) {
	...
	verify := func(ctx context.Context) error {
		return store.verifyMeasurementLineageV2(ctx, previousBytes, previous.options(t), current.artifact, last.options(t))
	}
	if err := verify(t.Context()); err != nil {
		t.Fatalf("strict real terminal gap replay failed: %v", err)
	}
	if history.retainedStartup || cfg.ProvisionalDeferClosedNativeInput {
		t.Fatal("strict gap used a provisional admission")
	}
	history.terminals[303].Transitions[0].Signature[0] ^= 1
	if err := verify(t.Context()); err == nil {
		t.Fatal("strict gap accepted a changed interior signature")
	}
	history.terminals[303].Transitions[0].Signature[0] ^= 1
	original := history.terminals[303]
	delete(history.terminals, 303)
	if err := verify(t.Context()); err == nil {
		t.Fatal("strict gap accepted a missing interior terminal")
	}
	history.terminals[303] = original
	store.v2.historyAdoption = nil
	if err := verify(t.Context()); err == nil {
		t.Fatal("ordinary strict owner gained the explicit adoption permission")
	}
	store.v2.historyAdoption = owner
	foreign := *owner
	history.historyAdoption = &foreign
	if err := verify(t.Context()); err == nil {
		t.Fatal("another startup owner supplied adoption authority")
	}
	after, err := canonicalReleaseMeasurementBytes(current.artifact)
	if err != nil || !bytes.Equal(currentBytes, after) {
		t.Fatalf("lineage rewrote signed current history: %v", err)
	}
}
