...
Need help? Email support@fullbars.xyz or visit https://github.com/full-bars/sn
`
...
func newDockerTopCmd() *cobra.Command {
	...
	}), "Live full-screen status dashboard for a provider container. Shows throughput graph, proxy pool state, client sessions, and node resources inside the container. Refreshes every 100ms–10s; press +/- to adjust rate, ? for help, q to quit.", "  urnet-docker top\n  urnet-docker top mynetwork-provider\n  urnet-docker top --unit mynetwork-provider")
}
...
func newDockerReportCmd() *cobra.Command {
	return withHelp(newCobraCmd("report <url> [target]", "set report URL inside container", nil, func(cmd *cobra.Command, args []string) error {
		return cmdDockerReport(args)
	}), "Set the report URL used by the provider inside the targeted container, applied without a restart.", "  urnet-docker report https://report.example.com --unit mynetwork-provider")
}
