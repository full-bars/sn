...
// releaseRepo is the GitHub "owner/repo" for release assets.
const releaseRepo = "full-bars/sn"

// dockerImage is the Docker image name used for container lookups.
const dockerImage = "3cape/urnetwork-provider"

// releaseAPIBase is the base GitHub API URL for releases (computed from releaseRepo).
var releaseAPIBase = "https://api.github.com/repos/" + releaseRepo

// releaseDownloadBase is the base download URL for release assets.
var releaseDownloadBase = "https://github.com/" + releaseRepo + "/releases/download"
...
func fetchLatestRelease() (*releaseInfo, error) {
	api := releaseAPIBase + "/releases/latest"
	...
	info := &releaseInfo{
		Tag:    rj.TagName,
		URL:    releaseDownloadBase + "/" + rj.TagName + "/urnetwork-provider-" + rj.TagName + ".tar.gz",
		Assets: rj.Assets,
	}
...
func fetchReleaseByTag(tag string) (*releaseInfo, error) {
	...
	api := releaseAPIBase + "/releases/tags/" + url.PathEscape(tag)
	client := &http.Client{Timeout: 30 * time.Second}
	req, err := http.NewRequest("GET", api, nil)
	if err != nil {
	...
	info := &releaseInfo{
		Tag:    tag,
		URL:    releaseDownloadBase + "/" + tag + "/urnetwork-provider-" + tag + ".tar.gz",
		Assets: rj.Assets,
	}
