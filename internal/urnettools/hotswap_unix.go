...
// an in-process verified handoff. If SIGUSR2 fails (e.g. old provider without a
// handler), falls back to the control socket trigger which works on all platforms.
func triggerHotSwap(p Provider) error {
	...
	if err := proc.Signal(syscall.SIGUSR2); err != nil {
		return fmt.Errorf("SIGUSR2 to provider PID %d failed: %w; control socket fallback is a separate explicit path", p.PID, err)
	}
	return nil
}
