...
func TestStatusServerConfig_HasSlowlorisTimeouts(t *testing.T) {
	...
	mainPath := filepath.Join(filepath.Dir(thisFile), "provide.go")
	src, err := os.ReadFile(mainPath)
	if err != nil {
		t.Fatalf("reading provide.go: %v", err)
	}

	block := regexp.MustCompile(`(?s)statusServer\s*:=\s*&http\.Server\{(.*?)\n\s*go func`).FindStringSubmatch(string(src))
	if len(block) < 2 {
		t.Fatal("could not locate the statusServer construction in provider/provide.go")
	}
