...
// - Provides minimal read-only JSON-RPC 2.0 client for EVM queries (eth_chainId, eth_call).
// - Exported aliases provided for public API surface while preserving unexported symbols for existing tests.
...
// EthRpcTimeout is the exported timeout bound for individual JSON-RPC requests.
const EthRpcTimeout = ethRpcTimeout
...
// EthRpcHexResult is the exported version of ethRpcHexResult.
func EthRpcHexResult(ctx context.Context, rpcUrl string, method string, params []any) (string, error) {
	return ethRpcHexResult(ctx, rpcUrl, method, params)
}
...
// ParseEthHexQuantity is the exported version of parseEthHexQuantity.
func ParseEthHexQuantity(hexQuantity string) (uint64, error) {
	return parseEthHexQuantity(hexQuantity)
}
...
// ParseEthHexBytes is the exported version of parseEthHexBytes.
func ParseEthHexBytes(hexData string) ([]byte, error) {
	return parseEthHexBytes(hexData)
}
