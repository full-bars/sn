...
// proxyProbeConcurrency is the default concurrency limit for background proxy probing.
...
// address may already be held by something longer (the proxy audit's park,
...
func selectURLProxiesToShed(state *ProxyState, traffic map[string]uint64, n int) []string {
	...
	if state != nil {
		for addr, e := range state.Proxies {
			...
			cands = append(cands, cand{addr, rank(e.Health), tx})
		}
	}
