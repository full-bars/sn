...
func (s *proxyEarningsStore) Observe(snapshot map[string]*bandwidth.ProxyBandwidth, now time.Time) {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	for key, bw := range snapshot {
		...
		// (ProxyBandwidthSnapshotByKey). Only the display format needs
		// normalizing down to its address; identity keys are already in
		// store key space and must pass through untouched — collapsing them
		// to a bare address would recreate the legacy-key collision this
		// store migrated away from.
		credKey := key
...
// new identity key (ProxySettings.Key()) for every address the given desired
// settings actually claim. Mirrors adoptLegacyProxyState's rules exactly (see
// its doc comment). This file has no separate Version field because its
// on-disk shape is a bare {address: entry} map; adoption is inferrable from
// the map's own keys, with no schema flag required.
