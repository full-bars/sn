...
func getHotSwapChildIPC() (*os.File, bool) {
	...
	if err := syscall.Fstat(3, &stat); err != nil {
		hotswapLog("⚠️ [hotswap] %s=1 is set but descriptor 3 is invalid (%v); starting as normal provider\n", EnvHotSwap, err)
		return nil, false
	}
	if stat.Mode&syscall.S_IFMT != syscall.S_IFSOCK {
		hotswapLog("⚠️ [hotswap] %s=1 is set but descriptor 3 is not a socket; starting as normal provider\n", EnvHotSwap)
		return nil, false
	}
