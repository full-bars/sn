...
// - Bundled hotSwapSocketpair implementation using syscall.ForkLock + syscall.Socketpair
//   + syscall.CloseOnExec for close-on-exec descriptor creation across all Unix platforms.
// - Replaced unexported tlog with hotswapLog.
...
func spawnHotSwapCandidate(exe string, args []string) (*HotswapParentSession, error) {
	// Close-on-exec prevents descriptor leakage into the child or unrelated forks (F-7).
	...
	if err := cmd.Start(); err != nil {
		...
		if errors.Is(err, os.ErrPermission) || strings.Contains(err.Error(), "permission denied") {
			hotswapLog("⚠️ [hotswap-heal] Candidate binary %s missing execute permission; auto-healing chmod 0755...\n", exe)
			if chmodErr := os.Chmod(exe, 0755); chmodErr == nil {
				...
				if retryErr := cmd.Start(); retryErr == nil {
					hotswapLog("⚡ [hotswap-heal] Successfully spawned candidate after permission repair (PID %d)\n", cmd.Process.Pid)
...
func getHotSwapChildIPCFromFd(fd int) (*os.File, bool) {
	...
	if err := syscall.Fstat(fd, &stat); err != nil {
		hotswapLog("⚠️ [hotswap] %s=1 is set but descriptor %d is invalid (%v); starting as normal provider\n", EnvHotSwap, fd, err)
		return nil, false
	}
	if stat.Mode&syscall.S_IFMT != syscall.S_IFSOCK {
		hotswapLog("⚠️ [hotswap] %s=1 is set but descriptor %d is not a socket; starting as normal provider\n", EnvHotSwap, fd)
		return nil, false
	}
...
func execInPlace(exe string, args []string, env []string) error {
	...
	for attempt := 1; attempt <= 3; attempt++ {
		...
		if errors.Is(err, syscall.ETXTBSY) {
			...
			hotswapLog("⚠️ [hotswap-heal] syscall.Exec hit ETXTBSY (attempt %d/3); retrying after %v...\n", attempt, backoff)
			time.Sleep(backoff)
			continue
		}
		break
