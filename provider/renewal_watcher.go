...
func newClientJWTStore(path string) *clientJWTStore {
	return &clientJWTStore{
		path:    path,
		entries: make(map[string]clientJWTEntry),
	}
}
...
func (s *clientJWTStore) loadLocked() {
	...
	if s.entries == nil {
		s.entries = make(map[string]clientJWTEntry)
	}
...
func (a *authRateLimiter) ReportResult(err error) {
	...
	if err == nil {
		...
		if a.successStreak >= authRateIncreaseThreshold {
			a.successStreak = 0
			newLimit := a.limiter.Limit() + authRateIncreaseStep
			if newLimit > a.max {
				newLimit = a.max
			}
	...
	if isRateLimitedError(err) {
		if time.Since(a.lastAdjustedAt) >= authRateAdjustCooldown {
			a.lastAdjustedAt = time.Now()
			newLimit := rate.Limit(float64(a.limiter.Limit()) * authRateDecreaseFactor)
			if newLimit < a.min {
				newLimit = a.min
			}
			a.limiter.SetLimit(newLimit)
