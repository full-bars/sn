...
func withTempHome(t *testing.T) string {
	...
	// Isolate the process-wide health registry too. A proxy that resamples or
	// relaunches keeps its health entry across tests; without a per-test reset,
	// assertions that watch ProxyHealthByAddress become order dependent when a
	// shuffled predecessor registers the same address.
...
// resetGlobalControlStateForTest is defined in control_socket_test.go
