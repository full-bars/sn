...
func refreshAuthSettings(addr, user string) *connect.ProxySettings {
...
// both, and the operator saw a prompt full of raw identity keys.
func TestPlanProxyRefresh_CredentialedProxyInSyncIsNotAChange(t *testing.T) {
	desired := []*connect.ProxySettings{
		refreshAuthSettings("gw.example:1080", "u1"),
		refreshAuthSettings("gw.example:1080", "u2"),
	}
...
func TestPlanProxyRefresh_AccountAtSharedGatewayChanges(t *testing.T) {
	desired := []*connect.ProxySettings{
		refreshAuthSettings("gw.example:1080", "u2"),
		refreshAuthSettings("gw.example:1080", "u4"),
		refreshAuthSettings("gw.example:1080", "u3"),
	}
	current := map[string]ProxyEntry{
...
// Unauthenticated proxies keep their bare-address key, and an empty health is
