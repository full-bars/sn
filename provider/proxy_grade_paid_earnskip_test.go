...
func seedEarnTracker(t *testing.T, addr string) {
	...
	// Production feeds the tracker ProxyBandwidthSnapshotByKey (identity
	// keys); feed the same identity key the grader looks up for the
	// credentialed "addr:u:p" config. Seeding a bare address would disable
	// earn-skip for credentialed proxies.
