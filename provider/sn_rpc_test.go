...
// sn_rpc_test.go — tests for minimal JSON-RPC eth_call and eth_chainId client.
//
// ADAPTATION NOTES (v2026 migration):
// - Ported from main to package provider.
// - Covers JSON-RPC 2.0 parsing, quantity decoding, hex bytes decoding, HTTP error handling,
//   timeout context cancellation, and RPC endpoint failover.
...
func TestParseEthHexQuantity(t *testing.T) {
	tests := []struct {
		...
		// and leaves bare hex untouched, so ParseUint then accepts it.
		{name: "no 0x prefix succeeds", input: "ff", want: 255},
		// parseEthHexQuantity only TrimPrefixes the lowercase "0x", so an
		// uppercase "0X" prefix is NOT stripped and must error.
...
func rpcBytesEqual(a, b []byte) bool {
	if len(a) != len(b) {
		return false
	}
	for i := range a {
