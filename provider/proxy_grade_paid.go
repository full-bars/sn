...
func runPaidProxyGradeOnce(ctx context.Context, apiHost string, apiPort uint16) {
	...
	func() {
		...
		if len(state.Proxies) > 0 {
			// state.Proxies is keyed by proxy identity (address, or address+user
			// for a shared-gateway credentialed proxy), so the loop variable is
			// a KEY; the dial address is its address part.
			for key, entry := range state.Proxies {
				...
				dialAddr, _ := connect.SplitProxyKey(key)
				ps := &connect.ProxySettings{Network: "tcp", Address: dialAddr}
	...
	type gradeResult struct {
		key              string // proxy.state identity key
		addr             string
		snapshotGradedAt time.Time
		user             string
		password         string
