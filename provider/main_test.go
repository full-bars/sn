...
// Adapted for v2026 API: backoffPacer(n, stagger, now, ctx) was replaced
// by backoffPacerWithDelay(baseDelay, slotDuration, ctx).
func TestBackoffPacer_ZeroStagger(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	// Zero delays must return true immediately
	start := time.Now()
	...
	// Canceled context with non-zero delays must return false.
	cancelCtx, cancelFn := context.WithCancel(context.Background())
	cancelFn()
	time.Sleep(5 * time.Millisecond) // let cancellation propagate
	ok = backoffPacerWithDelay(50*time.Millisecond, 10*time.Millisecond, cancelCtx)
	if ok {
