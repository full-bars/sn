...
// The identity that does NOT win the legacy entry must simply have no
// slow-retry record at all, not inherit a near-expired drop clock from an
...
func TestSlowRetryAdoptLegacy_SplitLoserStartsFreshNotDropped(t *testing.T) {
	...
	userA := jwtRegressionSettings(addr, "u1")
	userB := jwtRegressionSettings(addr, "u2")
...
func TestSlowRetryAdoptLegacy_NoAuthIsANoOp(t *testing.T) {
	...
	adopted, split := s.adoptLegacy([]*connect.ProxySettings{{Network: "tcp", Address: addr}})
	if adopted != 0 || split != 0 {
		t.Fatalf("adopted=%d split=%d, want 0,0", adopted, split)
	}
	if got := s.Proxies[addr]; got == nil || !got.StartedAt.Equal(started) {
