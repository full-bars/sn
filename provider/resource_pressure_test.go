...
func TestCurrentPressure_SetPressure(t *testing.T) {
	orig := currentPressure()
	t.Cleanup(func() { setPressure(orig) })

	setPressure(0.42)
	if v := currentPressure(); !almostEq(v, 0.42) {
		t.Fatalf("expected 0.42, got %v", v)
	}
	setPressure(0.85)
	if v := currentPressure(); !almostEq(v, 0.85) {
		t.Fatalf("expected 0.85, got %v", v)
	}
}

func TestApplyPressureMemoryBudget(t *testing.T) {
	orig := connect.MemoryBudget()
	t.Cleanup(func() { connect.SetMemoryBudget(orig) })

	// Calm (<0.5): budget reset to 0 (unscaled)
	applyPressureMemoryBudget(0.3)
	if b := connect.MemoryBudget(); b != 0 {
		t.Fatalf("expected 0 budget for pressure 0.3, got %v", b)
	}

	// Mid pressure (0.75): scaled down
	applyPressureMemoryBudget(0.75)
	b75 := connect.MemoryBudget()
	if b75 <= 0 || b75 >= 64<<20 {
		t.Fatalf("expected scaled budget between floor and ref, got %v", b75)
	}

	// Max pressure (1.0): floor (24 MiB)
	applyPressureMemoryBudget(1.0)
	if b := connect.MemoryBudget(); b != 24<<20 {
		t.Fatalf("expected floor 24MiB for pressure 1.0, got %v", b)
	}

	// Recovery back to calm: restores unscaled 0
	applyPressureMemoryBudget(0.1)
	if b := connect.MemoryBudget(); b != 0 {
		t.Fatalf("expected 0 budget on recovery, got %v", b)
	}
}

func TestReadMemAvailFrac(t *testing.T) {
	v, err := readMemAvailFrac()
	if err != nil {
		t.Skipf("readMemAvailFrac returned error on this platform: %v", err)
	}
	if v < 0 || v > 10.0 {
		t.Fatalf("readMemAvailFrac out of sane bounds: %v", v)
	}
}

func TestCollectPressureSample(t *testing.T) {
	s := collectPressureSample()
	if s.Goroutines < 1 {
		t.Fatalf("expected at least 1 goroutine, got %d", s.Goroutines)
	}
	// SensorErrs map must not be nil
	if s.SensorErrs == nil {
		t.Fatal("SensorErrs map must be initialized")
	}
}

func TestHostAvailMiB(t *testing.T) {
	v := hostAvailMiB()
	// On Linux /proc/meminfo should exist, so v >= 0
	if v < -1 {
		t.Fatalf("hostAvailMiB returned invalid value: %v", v)
	}
}

func TestLiveHeapFrac(t *testing.T) {
	f := liveHeapFrac()
	if f < 0 || f > 10.0 {
		t.Fatalf("liveHeapFrac out of sane bounds: %v", f)
	}
}

func TestGCAdaptiveEnabled(t *testing.T) {
	t.Setenv("GOGC", "off")
	if gcAdaptiveEnabled() {
		t.Fatal("expected disabled when GOGC is set")
	}

	t.Setenv("GOGC", "")
	t.Setenv("URNETWORK_ADAPTIVE_GC", "off")
	if gcAdaptiveEnabled() {
		t.Fatal("expected disabled when URNETWORK_ADAPTIVE_GC is off")
	}

	t.Setenv("URNETWORK_ADAPTIVE_GC", "1")
	if !gcAdaptiveEnabled() {
		t.Fatal("expected enabled when URNETWORK_ADAPTIVE_GC is 1")
	}
}

func TestApplyGCLevel(t *testing.T) {
	origGoGC := 100
	if goc, ok := readGOGCPercent(); ok {
		origGoGC = goc
	}
	...
	st := &gcGovernorState{baselineGOGC: 100, level: 1}
	applyGCLevel(st)
	if st.currentGOGC != 50 {
		t.Fatalf("expected currentGOGC=50 for level 1, got %d", st.currentGOGC)
	}
	if !gcTightening.Load() {
		t.Fatal("expected gcTightening=true for level 1")
	}

	st.level = 0
	applyGCLevel(st)
	if st.currentGOGC != 100 {
		t.Fatalf("expected currentGOGC=100 for level 0, got %d", st.currentGOGC)
	}
	if gcTightening.Load() {
		t.Fatal("expected gcTightening=false for level 0")
	}
}

func TestReadPSI(t *testing.T) {
	avg60, err := readPSI("memory")
	if err != nil {
		t.Skipf("readPSI not available on this host: %v", err)
	}
	if avg60 < 0 || avg60 > 100.0 {
		t.Fatalf("PSI avg60 out of bounds: %v", avg60)
	}
}

func TestFetchStretch(t *testing.T) {
	if v := fetchStretch(0); !almostEq(v, 1.0) {
		t.Fatalf("calm: expected 1.0, got %v", v)
	}
	if v := fetchStretch(0.9); !almostEq(v, fetchStretchMax) {
		t.Fatalf("full: expected %v, got %v", fetchStretchMax, v)
	}
	if v := fetchStretch(1.0); !almostEq(v, fetchStretchMax) {
		t.Fatalf("above full: expected %v, got %v", fetchStretchMax, v)
	}
}

func TestRunPressureMonitor_Cancellation(t *testing.T) {
	...
	case <-time.After(2 * time.Second):
		t.Fatal("runPressureMonitor did not exit promptly on canceled context")
	}
}

func TestRunPoolController_Cancellation(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	done := make(chan struct{})
	go func() {
		runPoolController(ctx, 100, false)
		close(done)
	}()
	select {
	case <-done:
	case <-time.After(2 * time.Second):
		t.Fatal("runPoolController did not exit promptly on canceled context")
	}
}
...
// Pool shed and the proxy audit's park each pick addresses from their own
