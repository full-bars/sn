...
// (proxy.state) is keyed by identity; the URL cache is keyed by bare address,
// so its lookup uses the key's address part. Precedence: the paid/file store (proxy.state, ProxyEntry) wins
...
// store has a graded entry for the key.
