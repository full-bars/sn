...
// auditLog formats and prints log messages for audit events with standard timestamps.
...
func initAuditRing() {
	...
	if ok, err := loadJSONWithRecovery(path, &saved); ok && len(saved.Entries) > 0 {
		...
		auditLog("[audit] error loading audit log: %v\n", err)
	}
...
// (field on the provide state) so a handoff successor is labelled as such.
...
func recordAndPersist(entry CommandAudit) {
	...
	if time.Since(lastAuditPersist) > 30*time.Second {
		lastAuditPersist = time.Now()
		if err := globalAuditRing.persist(); err != nil {
			auditLog("[audit] persist failed: %v\n", err)
		}
	}
}
...
func forceAuditPersist() {
	...
	lastAuditPersist = time.Now()
	if err := globalAuditRing.persist(); err != nil {
		auditLog("[audit] shutdown persist failed: %v\n", err)
	}
}
