...
func TestRuntime458ArtifactCheckerKeepsHistoricalProvenanceOffFreshRoute(t *testing.T) {
	...
	if err := decodeStrictJSONBytes(raw, &manifest); err != nil || len(manifest.Artifacts) != 6 {
...
func TestRuntime458SourceAttestationIncludesExactCompatibilityDelta(t *testing.T) {
	...
	for _, row := range strings.Split(strings.TrimSpace(string(previous)), "\n") {
		...
		if len(fields) != 2 || expectedKVs[fields[1]] != "" { t.Fatalf("malformed retained455 source row %q", row) }
		expectedKVs[fields[1]] = fields[0]
	}
	if len(expectedKVs) != 33 { t.Fatal("retained455 source scope changed") }
	...
	if err != nil { t.Fatal(err) }
	if len(expectedKVs) != 62 { t.Fatal("reviewed458 source scope changed") }
	if err := verifyReleaseRuntimeSourceCensus(string(current), expectedKVs); err != nil { t.Fatal(err) }
	checker, err := os.ReadFile("../scripts/check-runtime-v454-source.sh")
	if err != nil { t.Fatal(err) }
	for _, required := range []string{"for current_spec in 455 458", "runtime-v458-source.sha256", "current_commit=\"a7ae07e5dd37b552f27aa8e4d7716c522eef9aa7\"", "expected_current_files=62", "SUBTENSOR_RUNTIME458_SOURCE", "expected_files=29", "expected_metadata_files=18"} {
		if !strings.Contains(string(checker), required) { t.Fatalf("runtime458 source checker omits %q", required) }
	}
}
