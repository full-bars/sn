...
func TestFleetCommitmentHistoryRejectsAdjacentConsumers(t *testing.T) {
	for _, change := range []struct {
		...
		{name: "changed intent", mutate: func(f *fleetCommitmentHistoryFixture) { f.executor.journal.entries[2].IntentHash = common.Hash{9}.Hex() }},
		{name: "other consumer", mutate: func(f *fleetCommitmentHistoryFixture) { f.executor.journal.entries[2].ActionID = "fleet.install.batch.2" }},
...
func TestFleetCommitmentHistoryRejectsChangedApproval(t *testing.T) {
	for _, change := range []struct {
		...
		{name: "unapproved source", mutate: func(f *fleetCommitmentHistoryFixture) { f.approved.PriorPlanHashes = []string{f.consumerEntry.PlanHash} }},
		...
		{name: "duplicate original consumer", mutate: func(f *fleetCommitmentHistoryFixture) { f.executor.plan.Actions = append(f.executor.plan.Actions, f.consumer) }},
	} {
		fixture := newFleetCommitmentHistoryFixture(t, 1, 1)
		change.mutate(&fixture)
