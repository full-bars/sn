...
runtime=/home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/cohort1-normal-p2
...
declarations=/home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/inputs.sha256
...
root_list=/home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/inputs/cohort1.roots.txt
selector='^TestStClientKeyRegistrationCohortActualThousandClientsTwoOperators$'
outcomes=/home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/inputs/cohort1.outcomes.tsv
literals=/home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/inputs/cohort1.literals.tsv
binary=/home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/controller25-normal-p1/capture/controller-normal.testbin
package=controller
import_path=github.com/urnetwork/server/controller
...
if (( fence_status == 0 )); then
  if [[ -f /home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/controller25-normal-p1/outer/outer.exit && "$(< /home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/controller25-normal-p1/outer/outer.exit)" == 0 && -f "$binary.sha256" ]]; then
    sha256sum --check --strict --quiet "$binary.sha256" > "$capture/build.stdout" 2> "$capture/build.stderr"
    build_status=$?
    printf 'reuse original controller normal binary: %s\n' "$binary" > "$outer/build.command.txt"
  fi
fi
...
if (( list_status == 0 )); then
  printf '%s -test.run=%q -test.v=test2json -test.count=1 -test.parallel=4 -test.timeout=600s\n' "$binary" "$selector" > "$outer/body.command.txt"
  date -u +%Y-%m-%dT%H:%M:%S.%NZ > "$outer/body.started-at"
  (cd "$server/$package" && timeout --kill-after=30s 660s "$binary" "-test.run=$selector" -test.v=test2json -test.count=1 -test.parallel=4 -test.timeout=600s) > "$capture/body.stdout" 2> "$capture/body.stderr"
  body_status=$?
