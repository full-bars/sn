...
runtime=/home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/monitorfailed2-normal-p2
...
root_list=/home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/monitor-inputs/monitorfailed2.roots.txt
selector='^(TestMigrationsSignalReportsMissingPublishedArtifacts614ThroughHead|TestMigrationArtifactCatalogCoversEveryVersion614ThroughHead)$'
outcomes=/home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/monitor-inputs/monitorfailed2.outcomes.tsv
literals=/home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/monitor-inputs/monitorfailed2.literals.tsv
binary=/home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/monitor15-normal-p1/capture/monitor-normal.testbin
package=monitor
import_path=github.com/urnetwork/server/monitor
...
if (( fence_status == 0 )); then
  if [[ -f /home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/monitor15-normal-p1/outer/outer.exit && "$(< /home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/monitor15-normal-p1/outer/outer.exit)" == 0 && -f "$binary.sha256" ]]; then
    sha256sum --check --strict --quiet "$binary.sha256" > "$capture/build.stdout" 2> "$capture/build.stderr"
    build_status=$?
    printf 'reuse original monitor normal binary: %s\n' "$binary" > "$outer/build.command.txt"
  fi
fi
printf '%s\n' "$build_status" > "$outer/build.exit"
services_status=0
printf 'service-free monitor scope\n' > "$outer/services.stdout"
...
if (( list_status == 0 )); then
  printf '%s -test.run=%q -test.v=test2json -test.count=1 -test.parallel=4 -test.timeout=300s\n' "$binary" "$selector" > "$outer/body.command.txt"
  date -u +%Y-%m-%dT%H:%M:%S.%NZ > "$outer/body.started-at"
  (cd "$server/$package" && timeout --kill-after=30s 360s "$binary" "-test.run=$selector" -test.v=test2json -test.count=1 -test.parallel=4 -test.timeout=300s) > "$capture/body.stdout" 2> "$capture/body.stderr"
  body_status=$?
