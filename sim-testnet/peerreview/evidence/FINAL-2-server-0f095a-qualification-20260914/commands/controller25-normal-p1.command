...
runtime=/home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/controller25-normal-p1
...
root_list=/home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/inputs/controller25.roots.txt
selector='^TestStClientKey(RegistrationCohort|RegistrationReadiness|Publication|HistoryActualControl)'
outcomes=/home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/inputs/controller25.outcomes.tsv
literals=/home/by/urnetwork/temp/sn-server-migration-monitor-qualification-20260913/runtime/server-0f095a-v1/inputs/controller25.literals.tsv
...
if (( fence_status == 0 )); then
  printf 'go test -c -p=4 -o %s .\n' "$binary" > "$outer/build.command.txt"
  date -u +%Y-%m-%dT%H:%M:%S.%NZ > "$outer/build.started-at"
  (cd "$server/$package" && timeout --kill-after=30s 1200s go test -c -p=4 -o "$binary" .) > "$capture/build.stdout" 2> "$capture/build.stderr"
  build_status=$?
