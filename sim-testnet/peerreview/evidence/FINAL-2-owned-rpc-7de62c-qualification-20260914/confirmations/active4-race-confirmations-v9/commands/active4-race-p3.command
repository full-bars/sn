...
base="$runtime/active4-race-confirmations-v9"; id=active4-race-p3; directory="$base/confirmations/$id"
...
parent="$runtime/active4-race-confirmations-v7/confirmations/active4-race-p2"; parent_outer="$runtime/active4-race-confirmations-v7/outer/active4-race-p2.exit"
binary="$runtime/captures/active4-race-p1-v3/build-sim-testnet-race.testbin"; converter="$runtime/captures/active4-race-p1-v3/test2json"
...
[[ "$(cat "$parent_outer")" == 0 && "$(cat "$parent/result")" == pass ]] || exit 125
for stage in list body events; do [[ "$(cat "$parent/$stage.outer.exit")" == 0 && -f "$parent/$stage.joined" ]] || exit 125; done
[[ "$(cat "$parent/replay.exit")" == 0 ]] || exit 125
cmp -s "$parent/source.before.tsv" "$parent/source.after.tsv" || exit 125
cmp -s "$parent/binary.before.sha256" "$parent/binary.after.sha256" || exit 125
cmp -s "$parent/binary.before.stat" "$parent/binary.after.stat" || exit 125
cmp -s "$parent/converter.before.sha256" "$parent/converter.after.sha256" || exit 125
cmp -s "$parent/converter.before.stat" "$parent/converter.after.stat" || exit 125
write_state() {
