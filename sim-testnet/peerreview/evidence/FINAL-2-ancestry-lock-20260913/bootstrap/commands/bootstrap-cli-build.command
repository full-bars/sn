...
workspace="${FINAL_WORKSPACE:?FINAL_WORKSPACE is required}"
source_pair="${SOURCE_PAIR:?SOURCE_PAIR is required}"
expected_sn="${EXPECTED_SN:?EXPECTED_SN is required}"
warm_cache="${WARM_GOCACHE:?WARM_GOCACHE is required}"
modcache="${PRIVATE_GOMODCACHE:?PRIVATE_GOMODCACHE is required}"
source_root="$workspace/sn"
binary="$runtime/bin/sim-testnet-${expected_sn:0:7}-bootstrap"
snapshot() {
