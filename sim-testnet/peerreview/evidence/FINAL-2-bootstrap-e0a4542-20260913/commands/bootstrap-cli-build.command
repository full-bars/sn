...
expected_sn='e0a454248051a24d81c11d166054570cb4d20b5e'
binary="$runtime/bin/sim-testnet-e0a4542-bootstrap"
...
for private_path in "$runtime" "$runtime/tmp" "$runtime/gotmp" "$runtime/bin" "$warm_cache" "$modcache"; do
...
(
  ...
  exec env GOMAXPROCS=4 GOFLAGS='-mod=readonly -p=4' GOPROXY=off GOSUMDB=off GOWORK=off GOENV=off GOTOOLCHAIN=local GOVCS='*:off' TMPDIR="$runtime/tmp" GOTMPDIR="$runtime/gotmp" GOCACHE="$warm_cache" GOMODCACHE="$modcache" go build -trimpath -buildvcs=true -o "$binary" ./sim-testnet
...
printf '{\n  "kind":"temporary-bootstrap-stamped-sim-testnet-cli-build",\n  "build_exit":0,\n  "repos_before_after_identical":true,\n  "repos_before_sha256":"%s",\n  "repos_after_sha256":"%s",\n  "artifact":{"path":"%s","sha256":"%s","bytes":%s,"mode":"0700"},\n  "vcs":{"revision":"%s","trimpath":true,"buildvcs":true,"modified":false},\n  "warm_gocache":"%s",\n  "private_gomodcache":"%s"\n}\n' "$before_sha" "$after_sha" "$binary" "$binary_sha" "$binary_bytes" "$expected_sn" "$warm_cache" "$modcache" > "$runtime/RESULT-BOOTSTRAP-CLI-BUILD.json"
sha256sum "$runtime/RESULT-BOOTSTRAP-CLI-BUILD.json" > "$runtime/RESULT-BOOTSTRAP-CLI-BUILD.json.sha256"
