...
func precompileProbeSuccessorCallFixture(t *testing.T, fixture *precompileProbeSuccessorFixture) (*PrecompileConformanceEvidence, []JournalEntry, *deploymentBoundaryFixture) {
	...
	for index, actionId := range precompileProbeSuccessorTransactionIds() {
		...
		if index == 0 { receipt.ContractAddress = probe } else {
			var name string
	...
	if err := writePrecompileEvidence(fixture.stateDir, &evidence); err != nil { t.Fatal(err) }
...
func TestPrecompileProbeSuccessorRerendersAfterValuePhases(t *testing.T) {
	for _, consumed := range []int{2, 6} {
		...
		if ok, err := precompileProbeSuccessorNonce(fixture.plan, prefix, nonce); err != nil || !ok { t.Fatalf("completed value phase lost successor nonce admission: %v", err) }
		if err := verifyPrecompileProbeSuccessorCalls(t.Context(), fixture.cfg, fixture.stateDir, fixture.plan, prefix, reader, reader.finalized, nonce); err != nil { t.Fatalf("completed value phase lost exact finalized call proof: %v", err) }
		...
		if ok, err := coordinatorUpgradeMigrationNonceMatches(fixture.plan, migration, fixture.payloads, nonce, prefix); err != nil || !ok { t.Fatalf("completed value phase lost rerender observation: %v", err) }
	}
}
...
func truncatePrecompileProbeSuccessorEvidence(evidence *PrecompileConformanceEvidence, consumed int) {
	if consumed < 6 { evidence.Transfer = PrecompileTransferStep{}; evidence.Dividend = PrecompileDividendStep{}; evidence.Complete = false }
	if consumed < 5 { evidence.Snapshot = PrecompileSnapshotStep{} }
	if consumed < 4 { evidence.Back = PrecompileMoveStep{} }
	if consumed < 3 { evidence.Forward = PrecompileMoveStep{} }
}
...
func TestPrecompileProbeSuccessorRejectsCallPrefixDrift(t *testing.T) {
	...
	for _, change := range []string{"missing", "duplicate", "order", "intent", "plan", "deployment", "unrelated", "not finalized", "checkpoint", "extra nonce"} {
		...
		switch change {
		case "missing": entries = append(entries[:index], entries[index+1:]...)
		case "duplicate": copy := entries[index]; copy.TransactionHash = common.Hash{99}.Hex(); entries = append(entries, copy)
		case "order": entries[index].Sequence = entries[index-1].Sequence
		case "intent": entries[index].IntentHash = common.Hash{99}.Hex()
		case "plan": entries[index].PlanHash = common.Hash{99}.Hex()
		case "deployment": entries[index].DeploymentID = "synthetic-unapproved-deployment"
		case "unrelated": entries[index].ActionID = "unrelated.call"
		case "not finalized": entries[index].Stage = StageBroadcast
		case "checkpoint": entries[index].BlockHash = ""
		case "extra nonce": observed++
		}
		if ok, err := precompileProbeSuccessorNonce(fixture.plan, entries, observed); err == nil || ok { t.Fatalf("%s call prefix was accepted", change) }
	}
}
...
func TestPrecompileProbeSuccessorAcceptsExactRefinalization(t *testing.T) {
	...
	for _, consumed := range []int{2, 6} {
		...
		if err != nil || len(admitted) != consumed || admitted[len(admitted)-1].Sequence != first.Sequence { t.Fatalf("exact refinalization lost its first ordered receipt: %v", err) }
		if err := verifyPrecompileProbeSuccessorCalls(t.Context(), fixture.cfg, fixture.stateDir, fixture.plan, prefix, reader, reader.finalized, nonce); err != nil { t.Fatalf("exact refinalization lost canonical call proof: %v", err) }
		for _, change := range []string{"transaction", "block", "block hash", "recovery", "recovery hash", "intent", "plan"} {
			...
			switch change {
			case "transaction": last.TransactionHash = common.Hash{99}.Hex()
			case "block": last.BlockNumber++
			case "block hash": last.BlockHash = common.Hash{99}.Hex()
			case "recovery": last.RecoveryBlock++
			case "recovery hash": last.RecoveryBlockHash = common.Hash{99}.Hex()
			case "intent": last.IntentHash = common.Hash{99}.Hex()
			case "plan": last.PlanHash = common.Hash{99}.Hex()
			}
			if _, err := precompileProbeSuccessorPrefix(fixture.plan, changed, nonce); err == nil { t.Fatalf("changed %s refinalization was accepted", change) }
		}
	}
	create := slices.Clone(entries[:len(fixture.entries)+1])
	create = append(create, create[len(create)-1])
	if _, err := precompileProbeSuccessorPrefix(fixture.plan, create, fixture.plan.PrecompileProbeSuccessor.DeployerNonce+1); err == nil { t.Fatal("CREATE duplicate guard was weakened") }
}
...
func TestPrecompileProbeSuccessorRejectsChangedCallProof(t *testing.T) {
	for _, change := range []string{"signer", "nonce", "target", "calldata", "value", "chain", "pending", "failed receipt", "receipt hash", "canonical block", "evidence checkpoint", "event", "event address", "native source", "role", "dynamic amount"} {
		...
		if slices.Contains([]string{"signer", "nonce", "target", "calldata", "value", "chain"}, change) {
			...
			if err != nil { t.Fatal(err) }
			label := "deployer"
			if change == "signer" { label = "testnet-owner" }
			...
			switch change {
			case "nonce": call.Nonce++
			case "target": target := common.Address{99}; call.To = &target
			case "calldata": call.Data[len(call.Data)-1] ^= 1
			case "value": call.Value = new(big.Int).Add(call.Value, big.NewInt(1))
			case "chain": chainId.Add(chainId, big.NewInt(1))
			}
			...
			switch change {
			case "pending": reader.pending = true
			case "failed receipt": reader.receipts[hash].Status = types.ReceiptStatusFailed
			case "receipt hash": reader.receipts[hash].TxHash = common.Hash{99}
			case "canonical block": reader.blockHeads[entry.BlockNumber] = ChainHead{Number: entry.BlockNumber, Hash: common.Hash{99}.Hex()}
			case "evidence checkpoint": evidence.Seed.BlockNumber++
			case "event": reader.receipts[hash].Logs[0].Data[len(reader.receipts[hash].Logs[0].Data)-1] ^= 1
			case "event address": reader.receipts[hash].Logs[0].Address = common.Address{99}
			case "native source": evidence.CommitmentSource.PlanHash = common.Hash{99}.Hex()
			case "role": evidence.RecoveryColdkey = common.Hash{99}.Hex()
			case "dynamic amount": evidence.Forward.AmountRao++
			}
		}
		if err := writePrecompileEvidence(fixture.stateDir, evidence); err != nil { t.Fatal(err) }
		if err := verifyPrecompileProbeSuccessorCalls(t.Context(), fixture.cfg, fixture.stateDir, fixture.plan, entries, reader, reader.finalized, fixture.plan.PrecompileProbeSuccessor.DeployerNonce+6); err == nil { t.Fatalf("%s successor call proof was accepted", change) }
	}
}
...
func TestPrecompileProbeSuccessorResumesFinalizedCallBeforeEvidenceWrite(t *testing.T) {
	for _, consumed := range []int{2, 3, 4, 5, 6} {
		...
		case "precompile.seed": evidence.Seed.TransactionHash, evidence.Seed.BlockNumber, evidence.Seed.BlockHash = "", 0, ""; evidence.Seed.AfterRao, evidence.Seed.DeltaRao = 0, 0
		case "precompile.move-forward": evidence.Forward.TransactionHash, evidence.Forward.BlockNumber, evidence.Forward.BlockHash = "", 0, ""; evidence.Forward.FromAfterRao, evidence.Forward.ToAfterRao = 0, 0
		case "precompile.move-back": evidence.Back.TransactionHash, evidence.Back.BlockNumber, evidence.Back.BlockHash = "", 0, ""; evidence.Back.FromAfterRao, evidence.Back.ToAfterRao = 0, 0
		case "precompile.snapshot": evidence.Snapshot.TransactionHash, evidence.Snapshot.BlockNumber, evidence.Snapshot.BlockHash = "", 0, ""; evidence.Snapshot.SinceBlock = 0
		case "precompile.transfer-out": evidence.Transfer.TransactionHash, evidence.Transfer.BlockNumber, evidence.Transfer.BlockHash = "", 0, ""; evidence.Transfer.ProbeAfterRao, evidence.Transfer.ProviderAfterRao, evidence.Complete = 0, 0, false
		}
		if err := writePrecompileEvidence(fixture.stateDir, evidence); err != nil { t.Fatal(err) }
		nonce := fixture.plan.PrecompileProbeSuccessor.DeployerNonce+uint64(consumed)
		if err := verifyPrecompileProbeSuccessorCalls(t.Context(), fixture.cfg, fixture.stateDir, fixture.plan, prefix, reader, reader.finalized, nonce); err != nil { t.Fatalf("finalized %s lost recoverable input: %v", entry.ActionID, err) }
		if err := verifyPrecompileProbeSuccessorCall(t.Context(), reader, reader.finalized, fixture.plan, evidence, entry, nonce-1, false); err == nil { t.Fatalf("completed %s silently lost its evidence receipt", entry.ActionID) }
	}
}
