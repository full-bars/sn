...
func newCampaignSuccessionFixture(t *testing.T) *campaignSuccessionFixture {
	...
	if err := ensurePrivateDir(f.stateDir); err != nil { t.Fatal(err) }
	...
	if err := saveRoleSecrets(filepath.Join(f.stateDir, "secrets", "roles.json"), f.roles); err != nil { t.Fatal(err) }
	if err := writeRunInputs(f.cfg, f.stateDir, f.prior, f.roles); err != nil { t.Fatal(err) }
	...
	if err != nil { t.Fatal(err) }
	if err := f.attempt.updateProgress(false, true); err != nil { t.Fatal(err) }
...
func (f *campaignSuccessionFixture) writeResult(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if err := writePublicJSON(filepath.Join(f.stateDir, "runs", f.attempt.payload.RunID, "result.json"), f.result); err != nil { t.Fatal(err) }
}
...
func (f *campaignSuccessionFixture) writeCurrent(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if err := writeRunInputs(f.cfg, f.stateDir, f.current, f.roles); err != nil { t.Fatal(err) }
}
...
func TestScenarioCampaignAttemptSuccessionPreservesFailureAndRestartsPreparation(t *testing.T) {
	...
	if _, err := readScenarioCampaignAttempt(f.cfg, f.stateDir, f.roles, f.current.PlanHash, "release-1.0"); err == nil || !strings.Contains(err.Error(), "differs from the approved phase") { t.Fatalf("old-source causal control: %v", err) }
	if _, err := loadOrCreateScenarioCampaignAttempt(f.cfg, f.stateDir, f.roles, f.current.PlanHash, "release-1.0", nil, f.now); err == nil { t.Fatal("unowned old-source retry replaced the failure") }
	next, err := f.open()
	if err != nil { t.Fatal(err) }
	if next.payload.Succession == nil || next.payload.RunID == f.attempt.payload.RunID || next.payload.PreparationComplete || next.payload.HandoffAuthenticated || next.payload.AcceptanceBoundary != nil { t.Fatal("successor inherited predecessor progress") }
	if next.payload.Succession.PriorAttemptSHA256 != bytesSHA256(originalAttempt) || next.payload.Succession.PriorResultSHA256 != bytesSHA256(originalResult) { t.Fatal("successor did not pin exact original bytes") }
	prepared := 0
	if err := beginScenarioCampaignPreparation(t.Context(), "release-1.0", next.payload.RunID, scenarioRunOptions{Attempt: next, Prepare: func(context.Context) error { prepared++; return nil }}); err != nil { t.Fatal(err) }
	again, err := f.open()
	if err != nil || again.payload.RunID != next.payload.RunID || !again.payload.PreparationComplete || prepared != 1 { t.Fatalf("exact successor reopen: %v", err) }
	if !bytes.Equal(originalAttempt, readCampaignSuccessionFixtureBytes(t, legacy)) || !bytes.Equal(originalResult, readCampaignSuccessionFixtureBytes(t, resultPath)) || !bytes.Equal(originalJournal, readCampaignSuccessionFixtureBytes(t, filepath.Join(f.stateDir, "journal.jsonl"))) { t.Fatal("succession rewrote original failure or journal") }
}
...
func TestScenarioCampaignAttemptSuccessionRejectsUnknownLineageAndCustody(t *testing.T) {
	...
	for _, name := range []string{"unknown-plan", "same-plan", "missing-plan", "wrong-deployment", "wrong-owner", "forged-attempt", "missing-roles", "missing-provider", "unassigned-provider", "duplicate-provider"} {
		t.Run(name, func(t *testing.T) {
			f := newCampaignSuccessionFixture(t)
			switch name {
			case "unknown-plan": f.current.PriorPlanHashes = []string{"0x"+strings.Repeat("98", 32)}; f.writeCurrent(t)
			case "same-plan": f.current = f.prior; f.writeCurrent(t)
			case "missing-plan": if err := os.Remove(filepath.Join(f.stateDir, "plans", stringsTrim0x(f.prior.PlanHash)+".json")); err != nil { t.Fatal(err) }
			case "wrong-deployment": f.current.DeploymentID = "another-deployment"; f.writeCurrent(t)
			case "wrong-owner": role := f.roles.EVM["keeper"]; f.roles.EVM["testnet-owner"] = role
			case "forged-attempt":
				path := scenarioCampaignAttemptPath(f.stateDir, "release-1.0")
				var envelope ReleaseEvidenceEnvelope
				if err := json.Unmarshal(readCampaignSuccessionFixtureBytes(t, path), &envelope); err != nil { t.Fatal(err) }
				envelope.Signature = "0x"+strings.Repeat("00", 65)
				if err := writePublicJSON(path, envelope); err != nil { t.Fatal(err) }
			case "missing-roles": if err := os.Remove(filepath.Join(f.stateDir, "secrets", "roles.json")); err != nil { t.Fatal(err) }
			case "missing-provider": delete(f.roles.Clients, "miner-1000"); if err := saveRoleSecrets(filepath.Join(f.stateDir, "secrets", "roles.json"), f.roles); err != nil { t.Fatal(err) }
			case "unassigned-provider": role := f.roles.Clients["miner-1000"]; role.ClientIDHex = ""; f.roles.Clients["miner-1000"] = role; if err := saveRoleSecrets(filepath.Join(f.stateDir, "secrets", "roles.json"), f.roles); err != nil { t.Fatal(err) }
			case "duplicate-provider": role := f.roles.Clients["miner-1000"]; role.ClientIDHex = f.roles.Clients["miner-1"].ClientIDHex; f.roles.Clients["miner-1000"] = role; if err := saveRoleSecrets(filepath.Join(f.stateDir, "secrets", "roles.json"), f.roles); err != nil { t.Fatal(err) }
			}
			_, err := createScenarioCampaignSuccessor(f.cfg, f.stateDir, f.roles, f.current.PlanHash, f.now, f.journal)
			if err == nil { t.Fatal("invalid source admitted a successor") }
			if _, err := os.Lstat(scenarioCampaignSuccessorPath(f.stateDir)); !errors.Is(err, os.ErrNotExist) { t.Fatalf("refused succession published: %v", err) }
		})
	}
}
...
func TestScenarioCampaignAttemptSuccessionRejectsActiveAcceptedAndIncompletePredecessors(t *testing.T) {
	...
	for _, name := range []string{"active", "accepted", "accepted-file", "complete-file", "production-attempt", "started-lifecycle", "passing-result", "final-acceptance", "changed-result", "future-completion"} {
		t.Run(name, func(t *testing.T) {
			...
			switch name {
			case "active": if err := os.Remove(filepath.Join(run, "result.json")); err != nil { t.Fatal(err) }
			case "accepted":
				...
				if err := validateScenarioCampaignAttemptPayload(f.cfg, f.prior.PlanHash, "release-1.0", &f.attempt.payload); err != nil { t.Fatal(err) }
				if err := writeScenarioCampaignAttempt(f.attempt); err != nil { t.Fatal(err) }
			case "accepted-file": if err := atomicWrite(filepath.Join(run, scenarioCampaignStartFilename), []byte("retained acceptance"), 0o600); err != nil { t.Fatal(err) }
			case "complete-file": if err := atomicWrite(filepath.Join(run, "complete.json"), []byte("retained completion"), 0o600); err != nil { t.Fatal(err) }
			case "production-attempt": if err := atomicWrite(scenarioCampaignAttemptPath(f.stateDir, "production-soak"), []byte("retained production"), 0o600); err != nil { t.Fatal(err) }
			case "started-lifecycle": if err := atomicWrite(filepath.Join(f.stateDir, "public", "fleet-lifecycle.json"), []byte("retained lifecycle"), 0o600); err != nil { t.Fatal(err) }
			case "passing-result": f.result.Result = "pass"; f.writeResult(t)
			case "final-acceptance": accepted := true; f.result.FinalAcceptance = &accepted; f.writeResult(t)
			case "changed-result": f.result.Assertions[0].Message = "changed after hashing"; if err := writePublicJSON(filepath.Join(run, "result.json"), f.result); err != nil { t.Fatal(err) }
			case "future-completion": f.result.CompletedAt = f.now.Add(time.Hour).Format(time.RFC3339Nano); f.writeResult(t)
			}
			if _, err := f.open(); err == nil { t.Fatal("active, accepted or incomplete predecessor admitted") }
			if _, err := os.Lstat(scenarioCampaignSuccessorPath(f.stateDir)); !errors.Is(err, os.ErrNotExist) { t.Fatalf("refused succession published: %v", err) }
		})
	}
}
...
func TestScenarioCampaignAttemptSuccessionReopenRejectsChangedAndMissingSources(t *testing.T) {
	...
	for _, name := range []string{"attempt", "result", "prior-plan", "current-plan", "missing-prior-plan", "missing-result", "another-plan"} {
		t.Run(name, func(t *testing.T) {
			f := newCampaignSuccessionFixture(t)
			if _, err := f.open(); err != nil { t.Fatal(err) }
			...
			if strings.HasPrefix(name, "missing-") {
				if err := os.Remove(paths[strings.TrimPrefix(name, "missing-")]); err != nil { t.Fatal(err) }
			} else if name == "another-plan" {
				f.current.PriorPlanHashes = append(f.current.PriorPlanHashes, f.current.PlanHash); f.writeCurrent(t)
			} else {
				path := paths[name]
				if err := atomicWrite(path, append(readCampaignSuccessionFixtureBytes(t, path), ' '), 0o600); err != nil { t.Fatal(err) }
			}
			if _, err := f.open(); err == nil { t.Fatal("reopen silently replaced the approved successor or its source") }
			if !bytes.Equal(before, readCampaignSuccessionFixtureBytes(t, scenarioCampaignSuccessorPath(f.stateDir))) { t.Fatal("failed reopen rewrote its signed successor") }
		})
	}
}
...
func TestScenarioCampaignAttemptSuccessionRequiresExclusiveDeploymentOwner(t *testing.T) {
	...
	if other, err := OpenJournal(f.stateDir); err == nil { other.Close(); t.Fatal("active deployment owner was replaced") }
	if err := f.journal.Close(); err != nil { t.Fatal(err) }
	if _, err := f.open(); err == nil { t.Fatal("closed deployment owner signed a succession") }
	if _, err := os.Lstat(scenarioCampaignSuccessorPath(f.stateDir)); !errors.Is(err, os.ErrNotExist) { t.Fatal("unowned succession published") }
}
...
func installCampaignSuccessionLogFixture(t *testing.T, f *campaignSuccessionFixture) {
	...
	for _, path := range []string{stdout, stderr} { if err := atomicWrite(path, nil, 0o600); err != nil { t.Fatal(err) } }
	...
	if err != nil { t.Fatal(err) }
	if err := gate.Bind(state); err != nil { t.Fatal(err) }
	if err := writePublicJSON(filepath.Join(f.stateDir, "supervisor.json"), manifest); err != nil { t.Fatal(err) }
	if err := writePublicJSON(filepath.Join(f.stateDir, "supervisor.state.json"), state); err != nil { t.Fatal(err) }
}
	...
	t.Run("release-candidate", func(t *testing.T) {
		...
		err := runReleaseCandidateCampaignWithAnalyzer(t.Context(), f.cfg, f.stateDir, f.journal, &Executor{plan: f.current}, f.roles,
			func(ctx context.Context, _ *ResolvedConfig, _, phase string, _ *Journal, _ *Executor, attempt *scenarioCampaignAttempt) error {
				called = true
				if phase != "release-1.0" || attempt.payload.Succession == nil || attempt.payload.PreparationComplete { t.Fatal("composite did not begin a fresh full release") }
			...
			}, noOpCampaignPreflight, func(context.Context, *ResolvedConfig, string, string, *RoleSecrets, *ScenarioResult) error { return nil })
		if !called || !errors.Is(err, stop) { t.Fatalf("composite succession did not reach preparation: %v", err) }
	})
...
func TestScenarioCampaignAttemptSuccessionAcceptanceUsesSuccessorBytes(t *testing.T) {
	...
	if err := next.updateProgress(false, true); err != nil { t.Fatal(err) }
	...
	campaignStart.ObservationHash, _ = canonicalHashHex(campaignStart); baseline.ObservationHash, _ = canonicalHashHex(baseline)
	run := filepath.Join(f.stateDir, "runs", next.payload.RunID)
	if err := os.MkdirAll(run, 0o700); err != nil { t.Fatal(err) }
	for _, observation := range []*ScenarioObservation{campaignStart, baseline} { if err := appendObservation(filepath.Join(run, "observations.jsonl"), observation); err != nil { t.Fatal(err) } }
	...
	if err != nil { t.Fatal(err) }
	for i := range faults { if faults[i].PreAcceptance { faults[i].ArmedBlock = baseline.Status.Contracts.FinalizedHead.Number; faults[i].ArmedBlockHash = baseline.Status.Contracts.FinalizedHead.Hash } }
	...
	if err := next.bindAcceptanceBoundary(run, "0x"+strings.Repeat("77", 32), definitionHash, adversary, started.Add(2*time.Minute), campaignStart, baseline, window, faults); err != nil { t.Fatal(err) }
	if !bytes.Equal(readCampaignSuccessionFixtureBytes(t, next.path()), readCampaignSuccessionFixtureBytes(t, filepath.Join(run, scenarioCampaignStartFilename))) || !bytes.Equal(legacy, readCampaignSuccessionFixtureBytes(t, scenarioCampaignAttemptPath(f.stateDir, "release-1.0"))) { t.Fatal("acceptance copied legacy bytes or overwrote the failed attempt") }
	if window.EpochCount != 5 || window.EpochBlocks != 300 || window.FinalizeOffsetBlocks != 150 { t.Fatalf("succession reduced the full release window: %+v", window) }
}
