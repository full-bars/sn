...
func precompileProbeSuccessorPrefix(plan *SetupPlan, entries []JournalEntry, nonce uint64) ([]JournalEntry, error) {
	...
	for index, actionId := range ids {
		...
		for _, entry := range entries {
			...
			if entry.DeploymentID != plan.DeploymentID || !allowed[entry.PlanHash] || entry.IntentHash != action.IntentHash { return nil, fmt.Errorf("precompile probe successor %s has an unapproved finalized identity", actionId) }
			if matched != nil {
		...
		if index >= consumed {
			if matched != nil { return nil, errors.New("precompile probe successor finalized history exceeds observed nonce") }
			continue
		}
...
func precompileProbeSuccessorCall(plan *SetupPlan, evidence *PrecompileConformanceEvidence, actionId string) ([]byte, *big.Int, JournalEntry, error) {
	...
	if err != nil { return nil, nil, JournalEntry{}, err }
	...
	case "precompile.seed":
		...
		if value.String() != evidence.Seed.ValueWei { return nil, nil, recorded, errors.New("successor seed changed its approved value conversion") }
	...
	case "precompile.move-forward", "precompile.move-back":
		...
		if actionId == "precompile.move-back" { step, from, to, amount = evidence.Back, move, sample, evidence.Forward.AmountRao }
		if amount == 0 || step.AmountRao != amount || step.FromBeforeRao < amount { return nil, nil, recorded, errors.New("successor move differs from the approved round trip") }
	...
	case "precompile.snapshot":
		if evidence.Snapshot.BaselineRao == 0 || evidence.Snapshot.BaselineRao < evidence.Seed.AfterRao { return nil, nil, recorded, errors.New("successor snapshot lost the completed round trip") }
	...
	case "precompile.transfer-out":
		if evidence.Transfer.ProbeBeforeRao <= evidence.Seed.BeforeRao || evidence.Transfer.AmountRao != evidence.Transfer.ProbeBeforeRao-evidence.Seed.BeforeRao { return nil, nil, recorded, errors.New("successor transfer differs from the approved recovery") }
...
func verifyPrecompileProbeSuccessorCall(ctx context.Context, reader contractCreationReader, head ChainHead, plan *SetupPlan, evidence *PrecompileConformanceEvidence, entry JournalEntry, nonce uint64, allowPartial bool) error {
	...
	if recorded.TransactionHash == "" && recorded.BlockNumber == 0 && recorded.BlockHash == "" {
		if !allowPartial { return errors.New("successor completed prefix lost its recorded phase receipt") }
	...
	if err := verifyEVMCheckpointFromReader(ctx, reader, head, ChainHead{Number: entry.BlockNumber, Hash: entry.BlockHash}); err != nil { return err }
	receipt, err := reader.TransactionReceipt(ctx, common.HexToHash(entry.TransactionHash))
	if err != nil || !receiptMatchesEvidence(head, receipt, entry.TransactionHash, entry.BlockNumber, entry.BlockHash) { return stateMismatchError(err, "successor call has no exact canonical successful receipt") }
	transaction, pending, err := reader.TransactionByHash(ctx, receipt.TxHash)
	chainId := new(big.Int).SetUint64(plan.ChainID)
	if err != nil || transaction == nil || pending || transaction.Hash() != receipt.TxHash || !transaction.Protected() || transaction.ChainId().Cmp(chainId) != 0 { return stateMismatchError(err, "successor call changed its signed chain or inclusion") }
	signer, err := types.Sender(types.LatestSignerForChainID(chainId), transaction)
	if err != nil || signer != common.HexToAddress(plan.Roles.Deployer) || transaction.Nonce() != nonce || transaction.To() == nil || *transaction.To() != common.HexToAddress(plan.PrecompileProbeSuccessor.Probe) || transaction.Value().Cmp(value) != 0 || !bytes.Equal(transaction.Data(), data) { return stateMismatchError(err, "successor call differs from its exact approved signer, nonce, target, value or calldata") }
...
func verifyPrecompileProbeSuccessorEvent(evidence *PrecompileConformanceEvidence, actionId string, receipt *types.Receipt, partial bool) error {
	...
	for _, log := range receipt.Logs {
		if log != nil && log.Address == common.HexToAddress(evidence.ProbeAddress) { filtered.Logs = append(filtered.Logs, log) }
	}
	...
	case "precompile.seed":
		...
		if !ok || value.String() != evidence.Seed.ValueWei { return errors.New("successor seed event changed the approved value") }
		if !partial {
			fields["stakeAfter"] = evidence.Seed.AfterRao
			if evidence.Seed.DeltaRao == 0 || !exactIncrease(evidence.Seed.BeforeRao, evidence.Seed.AfterRao, evidence.Seed.DeltaRao) { return errors.New("successor seed event changed its exact delta") }
		}
	case "precompile.move-forward", "precompile.move-back":
		step, from, to := evidence.Forward, sample, move
		if actionId == "precompile.move-back" { step, from, to = evidence.Back, move, sample }
		...
		if !partial { fields["fromAfter"], fields["toAfter"] = step.FromAfterRao, step.ToAfterRao }
	case "precompile.snapshot":
		...
		if !partial { fields["blockNumber"] = evidence.Snapshot.SinceBlock }
	case "precompile.transfer-out":
		...
		if !partial { fields["sourceAfter"], fields["destinationAfter"] = evidence.Transfer.ProbeAfterRao, evidence.Transfer.ProviderAfterRao }
	default:
		return errors.New("successor receipt names an unapproved event")
	}
	if err != nil { return err }
	for name, want := range fields {
		if got, ok := conformanceEventUint64(values, name); !ok || got != want { return fmt.Errorf("successor %s event changed %s", actionId, name) }
	}
	return nil
}
...
func verifyPrecompileProbeSuccessorCalls(ctx context.Context, cfg *ResolvedConfig, stateDir string, plan *SetupPlan, entries []JournalEntry, reader contractCreationReader, head ChainHead, nonce uint64) error {
	...
	if err != nil || len(prefix) <= 1 { return err }
	evidence, err := loadPrecompileEvidence(stateDir)
	if err != nil { return err }
	if err := validatePrecompileEvidenceIdentity(cfg, common.HexToAddress(plan.PrecompileProbeSuccessor.Probe), evidence); err != nil { return err }
	if _, err := precompileProbeSuccessorEvidence(plan, evidence, evidence); err != nil { return err }
	old := plan.PrecompileProbeSuccessor.Evidence
	if evidence.Owner != old.Owner || evidence.SampleHotkey != old.SampleHotkey || evidence.SampleUID != old.SampleUID || evidence.AbsentHotkey != old.AbsentHotkey || evidence.MoveHotkey != old.MoveHotkey || evidence.RecoveryColdkey != old.RecoveryColdkey { return errors.New("successor call evidence changed the original approved roles") }
	...
	for index, entry := range prefix[1:] {
		...
		if complete {
			if _, err := owner.readPersistedPostcondition(verified); err != nil { return err }
		}
		if err := verifyPrecompileProbeSuccessorCall(ctx, reader, head, plan, evidence, entry, plan.PrecompileProbeSuccessor.DeployerNonce+uint64(index)+1, index == len(prefix)-2 && !complete); err != nil { return err }
	}
	return nil
}
