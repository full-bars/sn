...
func validateStrictHistoryAdoptionOptions(command string, options cliOptions) error {
	...
	if command == "history-adoption" {
		if requested || options.Apply || options.ProvisionalResume || options.FirstNativeEpoch == 0 {
			return errors.New("history-adoption requires a fresh explicit native epoch and read-only strict mode")
		}
	}
...
func strictHistoryAdoptionInputs(ctx context.Context, cfg *ResolvedConfig, stateDir string, plan *SetupPlan) (*ResolvedConfig, *RoleSecrets, map[string]any, string, string, string, error) {
	...
	if prepared.PlanHash == plan.PlanHash || !plan.allowedPlanHashes()[prepared.PlanHash] {
		return nil, nil, nil, "", "", "", errors.New("strict history adoption requires its approved original activation ancestor")
	}
...
func strictHistorySupervisorStopped(stateDir string) error {
	...
	if err == nil {
		if err := syscall.Flock(int(lock.Fd()), syscall.LOCK_EX|syscall.LOCK_NB); err != nil {
			return errors.Join(errors.New("strict history adoption requires a stopped supervisor lock"), err, lock.Close())
		}
		if err := errors.Join(syscall.Flock(int(lock.Fd()), syscall.LOCK_UN), lock.Close()); err != nil {
			return err
		}
	}
	...
	if active != nil {
		return errors.New("strict history adoption requires the recorded supervisor to be stopped")
	}
	return nil
}
...
func readStrictHistoryAdoptionFile(stateDir, path string, maximum int64) ([]byte, error) {
	if !filepath.IsAbs(path) || filepath.Clean(path) != path {
		return nil, errors.New("strict history request path must be clean and absolute")
	}
	relative, err := filepath.Rel(stateDir, path)
	if err != nil || !strings.HasPrefix(filepath.ToSlash(relative), "history-adoptions/") {
		return nil, errors.New("strict history request is outside its simulator provenance")
	}
...
func prepareStrictHistoryAdoption(ctx context.Context, cfg *ResolvedConfig, stateDir string, options cliOptions, plan *SetupPlan) error {
	if options.StrictHistoryAdoption == "" {
		return nil
	}
	...
	if err != nil || bytesSHA256(raw) != options.StrictHistoryAdoptionSHA256 {
		return errors.Join(errors.New("strict history adoption request changed its approval pin"), err)
	}
	...
	if err := preflightStrictHistoryAdoption(ctx, cfg, stateDir); err != nil {
		cfg.strictHistoryAdoption = previous
		return err
	}
	return nil
}
...
func preflightStrictHistoryAdoption(ctx context.Context, cfg *ResolvedConfig, stateDir string) error {
	...
	if err != nil || bytesSHA256(raw) != state.hash {
		return errors.Join(errors.New("strict history adoption bundle changed after admission"), err)
	}
	plan, err := loadPersistedPlan(cfg, stateDir)
	if err != nil {
		return err
	}
	if plan.PlanHash != state.bundle.ApprovedPlanHash {
		return errors.New("strict history adoption approval was replaced")
	}
	resolved, roles, base, source, prepared, completed, err := strictHistoryAdoptionInputs(ctx, cfg, stateDir, plan)
	if err != nil {
		return err
...
func attachStrictHistoryAdoption(cfg *ResolvedConfig, stateDir string, plan *SetupPlan, specs []ProcessSpec) error {
	...
	if plan.PlanHash != cfg.strictHistoryAdoption.bundle.ApprovedPlanHash {
		return errors.New("strict history process owner changed approved plan")
	}
	if err := preflightStrictHistoryAdoption(context.Background(), cfg, stateDir); err != nil {
		return err
	}
	for index, raw := range cfg.strictHistoryAdoption.requests {
		...
		if err != nil || configHash != request.ConfigSHA256 {
			return errors.Join(errors.New("strict history actual rendered config differs from approved bytes"), err)
		}
		...
		if previous, err := os.ReadFile(path); err == nil && !bytes.Equal(previous, raw) {
			return errors.New("strict history immutable child request was replaced")
		...
		if found != 1 {
			return errors.New("strict history handoff requires one exact validator process owner")
		}
	}
	return nil
}
