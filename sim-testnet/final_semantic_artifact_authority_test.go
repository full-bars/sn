...
func TestFinalPayoutArtifactSeparatesProvenanceFromRootAuthorization(t *testing.T) {
	...
	if strings.EqualFold(fixture.expected.ArtifactSigner.Hex(), fixture.pool.PayoutRootSigner) { t.Fatal("fixture keys are not distinct") }
	...
	if err != nil { t.Fatal(err) }
	if err := payoutartifact.Sign(artifact, rootKey); err != nil { t.Fatal(err) }
	rootSigned, err := payoutartifact.Bytes(artifact)
	if err != nil { t.Fatal(err) }
	if _, err := payoutartifact.Decode(rootSigned); err != nil { t.Fatalf("negative control is not validly signed: %v", err) }
...
func TestFinalPayoutArtifactConfiguredRoleCensusRejectsSubstitutions(t *testing.T) {
	...
	for _, name := range []string{"valid", "missing", "malformed", "zero", "reused", "deployment", "schema"} {
		t.Run(name, func(t *testing.T) {
			...
			switch name {
			case "missing": delete(identities.EVM, "operator-1-artifact")
			case "malformed": identities.EVM["operator-1-artifact"] = "not-an-address"
			case "zero": identities.EVM["operator-1-artifact"] = common.Address{}.Hex()
			case "reused": identities.EVM["operator-2-artifact"] = identities.EVM["operator-1-artifact"]
			case "deployment": identities.DeploymentID = "another-deployment"
			case "schema": identities.Schema = "unknown"
			}
			signers, err := finalOperatorArtifactSigners(identities, "artifact-authority", 2)
			if name == "valid" {
				if err != nil || len(signers) != 2 || signers[1] != (common.Address{1}) || signers[2] != (common.Address{2}) { t.Fatalf("exact retained artifact roles: %v", err) }
			} else if err == nil { t.Fatal("invalid artifact role source was admitted") }
		})
	}
}
...
func TestFinalPayoutArtifactSignedAuditProvenanceBindsAllDecisionSources(t *testing.T) {
	...
	for _, name := range []string{"ordinary", "penalty", "recovery"} {
		t.Run(name, func(t *testing.T) {
			...
			switch name {
			case "ordinary": evidence.Validators = []FinalValidatorIdentityEvidence{{Cycles: []FinalCRv4Cycle{cycle}}}; target = &evidence.Validators[0].Cycles[0]
			case "penalty": evidence.DishonestDeposit = &FinalDishonestDepositEvidence{Penalties: []FinalDishonestDepositDecision{{Cycle: cycle}}}; target = &evidence.DishonestDeposit.Penalties[0].Cycle
			case "recovery": evidence.DishonestDeposit = &FinalDishonestDepositEvidence{Recoveries: []FinalDishonestDepositDecision{{Cycle: cycle}}}; target = &evidence.DishonestDeposit.Recoveries[0].Cycle
			}
			signers := map[uint64]common.Address{1: artifactSigner}
			if err := verifyFinalArtifactSignerProvenance(evidence, signers); err != nil { t.Fatal(err) }
			target.Pools[0].ArtifactSigner = target.Pools[0].RootSigner
			if err := verifyFinalArtifactSignerProvenance(evidence, signers); err == nil { t.Fatal("audit changed its configured artifact signer to root authority") }
		})
	}
}
...
func TestFinalPayoutArtifactCollectorUsesRetainedSigner(t *testing.T) {
	...
	for _, name := range []string{"configured", "root-key", "other-operator"} {
		t.Run(name, func(t *testing.T) {
			...
			if err != nil { t.Fatal(err) }
			if name == "root-key" { fixture.key = rootKey }
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				if r.Method != http.MethodGet || r.Host != "127.0.0.1:18081" || r.URL.Path != "/sn/artifact" { http.Error(w, "wrong request", http.StatusBadRequest); return }
			...
			if name != "configured" {
				if err == nil || !strings.Contains(err.Error(), "payout artifact is missing") { t.Fatalf("foreign artifact signer was collected: %v", err) }
				if entries, readErr := os.ReadDir(runRoot); readErr != nil || len(entries) != 0 { t.Fatalf("foreign signature wrote artifact bytes: %v", readErr) }
				return
			}
			if err != nil || len(collected) != 2 || len(lifecycle) != 0 { t.Fatalf("distinct artifact/root collector: %d/%d %v", len(collected), len(lifecycle), err) }
			for index, item := range collected {
				if item.NoID != 1 || item.Epoch != uint64(index+9) { t.Fatal("collector changed the complete source/accepted epoch census") }
				raw, readErr := os.ReadFile(filepath.Join(runRoot, item.Artifact.URI))
				if readErr != nil || !bytes.Equal(raw, bodies[hashes[index]]) { t.Fatalf("collector changed canonical signed bytes: %v", readErr) }
			}
		})
	}
}
