...
func coordinatorRepairHistoryRequired(plan *SetupPlan, entries []JournalEntry) bool {
	...
	for _, entry := range entries {
		if plan.allowedPlanHashes()[entry.PlanHash] && (entry.ActionID == "repair.coordinator-rounding.deploy" || entry.ActionID == "repair.coordinator-rounding.activate") {
			return true
		}
	}
	return false
...
func validateCoordinatorRepairCarryPlan(plan *SetupPlan) error {
	if plan == nil {
		return errors.New("coordinator repair plan is absent")
	}
	...
	if err := verifyCoordinatorRepairSignature(r, carry.Request.Hash, carry.Request.Signature, r.Owner); err != nil {
		return err
	}
	if err := validateCoordinatorRepairCarryResult(carry); err != nil {
		return err
	}
	if result.Deploy.PlanHash != r.PlanHash || result.Activate.PlanHash != r.PlanHash {
		return errors.New("coordinator repair carry crosses original action approvals")
	}
	return nil
}
...
func validateCoordinatorRepairCarryResult(carry *CoordinatorRepairCarry) error {
	if carry == nil {
		return errors.New("coordinator repair carry is absent")
	}
	r, result := carry.Request.Request, carry.Result.Result
	...
	for i, entry := range []JournalEntry{result.Deploy, result.Activate} {
		...
		if entry.Stage != StageFinalized || entry.DeploymentID != r.DeploymentID || entry.PlanHash != r.PlanHash || entry.ActionID != action.ID || entry.IntentHash != action.IntentHash || entry.BlockNumber == 0 || !validCanonicalHashHex(entry.BlockHash) || !validCanonicalHashHex(entry.TransactionHash) {
			return errors.New("coordinator repair result lacks its exact finalized action")
		}
	}
...
func readCoordinatorRepairCarry(stateDir string, plan *SetupPlan, entries []JournalEntry) (*coordinatorRepairCarryObservation, error) {
	...
	if errors.Is(err, os.ErrNotExist) && !coordinatorRepairHistoryRequired(plan, entries) {
		return nil, nil
	}
	if err != nil {
		return nil, err
	...
	if err := decodeExactCoordinatorRepairJSON(requestRaw, &reference.Request); err != nil {
		return nil, err
	}
	...
	if err := decodeExactCoordinatorRepairJSON(resultRaw, &reference.Result); err != nil {
		return nil, err
	}
	r := reference.Request.Request
	if !plan.allowedPlanHashes()[r.PlanHash] {
		return nil, errors.New("coordinator repair source is outside approved lineage")
	}
	...
	if baseline != source.CoordinatorUpgradeBaseline {
		return nil, errors.New("coordinator repair changed the original precompile baseline")
	}
	if err := validateCoordinatorRepairRequest(source, &reference.Request); err != nil {
		return nil, err
	}
	if err := validateCoordinatorRepairCarryResult(&reference); err != nil {
		return nil, err
	}
	if plan.CoordinatorRepairCarry != nil {
		...
		if !reflect.DeepEqual(*plan.CoordinatorRepairCarry, reference) {
			return nil, errors.New("coordinator repair source bytes differ from approved carry")
		}
	} else if plan.CoordinatorUpgrade != r.OldUpgrade {
		return nil, errors.New("coordinator repair original upgrade differs")
	}
	...
	if crypto.Keccak256Hash(runtime).Hex() != r.Upgrade.RuntimeCodeHash {
		return nil, errors.New("coordinator repair runtime differs from signed artifact")
	}
	...
	if !reflect.DeepEqual(wantDeploy, r.Deploy) || !reflect.DeepEqual(wantActivate, r.Activate) {
		return nil, errors.New("coordinator repair signed actions differ from exact CREATE and activation bytes")
	}
	observation := &coordinatorRepairCarryObservation{reference: reference, source: source, creation: creation, runtime: runtime}
	anchor := uint64(0)
	for _, entry := range entries {
		if entry.EntryHash == r.Budget.JournalHash && entry.PlanHash == source.PlanHash {
			if anchor != 0 {
				return nil, errors.New("coordinator repair budget anchor is ambiguous")
			}
			anchor = entry.Sequence
		}
	}
	if anchor == 0 {
		return nil, errors.New("coordinator repair budget lacks its original journal anchor")
	}
	for i, final := range []JournalEntry{reference.Result.Result.Deploy, reference.Result.Result.Activate} {
		...
		for _, entry := range entries {
			...
			if entry.IntentHash != action.IntentHash || entry.DeploymentID != source.DeploymentID {
				return nil, errors.New("coordinator repair journal has competing action identity")
			}
			if entry.Stage == StageFinalized {
				if entry != final {
					return nil, errors.New("coordinator repair finalized journal entry changed")
				}
				finals++
			}
			if entry.Stage == StageBroadcast {
				if broadcast != nil || entry.TransactionHash != final.TransactionHash || entry.Sequence <= anchor || entry.Sequence >= final.Sequence {
					return nil, errors.New("coordinator repair original broadcast is ambiguous")
				}
				copy := entry
				broadcast = &copy
			}
		}
		if finals != 1 || broadcast == nil {
			return nil, errors.New("coordinator repair lacks one original broadcast and finalization")
		}
...
func decodeExactCoordinatorRepairJSON(raw []byte, target any) error {
	if _, err := decodeOrderedJSONObject(raw); err != nil {
		return err
	}
	...
	if err := decoder.Decode(&extra); !errors.Is(err, io.EOF) {
		return errors.New("coordinator repair JSON has trailing content")
	}
	return nil
}
...
func verifyCoordinatorRepairReceipt(ctx context.Context, client *ethclient.Client, head ChainHead, final JournalEntry, transaction *types.Transaction, created common.Address) error {
	if err := verifyEVMCheckpointFromReader(ctx, ethEVMReceiptFinalityReader{client: client}, head, ChainHead{Number: final.BlockNumber, Hash: final.BlockHash}); err != nil {
		return err
	}
	...
	if !receiptMatchesEvidence(head, receipt, final.TransactionHash, final.BlockNumber, final.BlockHash) || receipt.ContractAddress != created {
		return errors.New("coordinator repair receipt differs from successful canonical inclusion")
	}
	...
	if actual == nil || pending {
		return errors.New("coordinator repair included transaction is absent or pending")
	}
	...
	if err != nil || !bytes.Equal(want, got) {
		return errors.New("coordinator repair inclusion differs from signed bytes")
	}
	return nil
}
...
func authenticateCoordinatorRepairCarry(ctx context.Context, cfg *ResolvedConfig, stateDir string, plan *SetupPlan, entries []JournalEntry, operational, independent *ethclient.Client) (*coordinatorRepairCarryObservation, error) {
	if ctx == nil || cfg == nil || plan == nil || operational == nil || independentRPCRequired(cfg) && independent == nil {
		return nil, errors.New("coordinator repair reader ownership is incomplete")
	}
	...
	for _, client := range []*ethclient.Client{operational, independent} {
		...
		if err != nil || chainID == nil || chainID.Cmp(new(big.Int).SetUint64(plan.ChainID)) != 0 {
			return nil, stateMismatchError(err, "coordinator repair provider has another chain")
		}
		head, err := finalizedEVMHead(ctx, client)
		if err != nil {
			return nil, err
		}
		if err := verifyEVMCheckpointFromReader(ctx, ethEVMReceiptFinalityReader{client: client}, head, result.ObservedHead); err != nil {
			return nil, err
		}
		...
		for _, address := range contractDeploymentAddresses(observation.source.Deployment) {
			...
			if err != nil || len(code) == 0 || !strings.EqualFold(crypto.Keccak256Hash(code).Hex(), observation.source.Deployment.RuntimeHashes[address.Hex()]) {
				return nil, stateMismatchError(err, "coordinator repair changed immutable custody runtime at %s", address)
			}
		}
		...
		if probe != observation.source.Deployment.PrecompileProbe {
			...
			if err != nil || len(code) == 0 || !strings.EqualFold(crypto.Keccak256Hash(code).Hex(), observation.source.CoordinatorUpgradeBaseline.ReplacementPrecompileProbeHash) {
				return nil, stateMismatchError(err, "coordinator repair changed retained replacement probe")
			}
		}
		for i, final := range []JournalEntry{result.Deploy, result.Activate} {
			created := common.Address{}
			if i == 0 {
				created = r.Upgrade.Implementation
			}
			if err := verifyCoordinatorRepairReceipt(ctx, client, head, final, observation.transactions[i], created); err != nil {
				return nil, err
			}
			...
			if err != nil || !bytes.Equal(code, observation.runtime) {
				return nil, stateMismatchError(err, "coordinator repair historical runtime changed")
			}
			if i == 1 {
				active, err := implementationAt(ctx, manager, r.Proxy, at)
				if err != nil || active != r.Upgrade.Implementation {
					return nil, stateMismatchError(err, "coordinator repair historical activation differs")
				}
				identity, err := coordinatorRepairIdentity(ctx, manager, observation.source, at)
				if err != nil || identity != r.IdentityHash {
					return nil, stateMismatchError(err, "coordinator repair historical custody differs")
				}
			}
		}
		code, err := codeAtCanonicalHead(ctx, manager, r.Upgrade.Implementation, head)
		if err != nil || !bytes.Equal(code, observation.runtime) {
			return nil, stateMismatchError(err, "coordinator repair current runtime changed")
		}
		active, err := implementationAt(ctx, manager, r.Proxy, head)
		if err != nil || active != r.Upgrade.Implementation {
			return nil, stateMismatchError(err, "coordinator repair current activation differs")
		}
		...
		if err != nil || pending != nonce {
			return nil, stateMismatchError(err, "coordinator repair deployer still has pending writes")
		}
...
func observeCoordinatorRepairCarry(ctx context.Context, cfg *ResolvedConfig, stateDir string, plan *SetupPlan, entries []JournalEntry) (*coordinatorRepairCarryObservation, error) {
	if ctx == nil || cfg == nil || plan == nil {
		return nil, errors.New("coordinator repair planning owner is absent")
	}
	...
	if _, err := readValidatorEvidenceHistoricalFile(stateDir, coordinatorRepairDirectory+"/request.json", 128<<10); errors.Is(err, os.ErrNotExist) && !coordinatorRepairHistoryRequired(plan, entries) {
		return nil, nil
	} else if err != nil {
		return nil, err
	}
...
func coordinatorRepairBaselineUpgrade(plan *SetupPlan) CoordinatorUpgrade {
	if plan.CoordinatorRepairCarry != nil {
		return plan.CoordinatorRepairCarry.Request.Request.OldUpgrade
	}
...
func bindCoordinatorRepairCarryPayloads(payloads *DeploymentPayloads, observation *coordinatorRepairCarryObservation) error {
	if payloads == nil || observation == nil || observation.source == nil {
		return errors.New("coordinator repair payload owner is absent")
	}
	r := observation.reference.Request.Request
	if payloads.CoordinatorUpgrade != r.Upgrade || !bytes.Equal(payloads.UpgradeImplementation, observation.creation) || !bytes.Equal(payloads.ExpectedRuntime[r.Upgrade.Implementation], observation.runtime) {
		return errors.New("completed coordinator repair differs from the current locked release")
	}
	...
	if err != nil || nonce != r.OldUpgrade.DeployerNonce+1 {
		return errors.New("coordinator repair retained batcher boundary differs")
	}
	if err := configureFleetBatcherNonce(payloads, nonce); err != nil {
		return err
	}
	if common.HexToAddress(batcher.Target) != payloads.FleetBatcherAddress || !strings.EqualFold(batcher.Parameters["runtime_code_hash"], crypto.Keccak256Hash(payloads.FleetBatcherRuntime).Hex()) {
		return errors.New("coordinator repair retained batcher differs from locked release")
	}
	return nil
}
...
func coordinatorRepairCarryMigration(prior *SetupPlan, current *SetupFacts, built *DeploymentPayloads, entries []JournalEntry) (*coordinatorUpgradeMigration, error) {
	observation := prior.coordinatorRepairObserved
	if observation == nil || observation.source == nil || current.DeployerNonce != observation.deployerNonce {
		return nil, errors.New("coordinator repair migration lost its finalized observation")
	}
	...
	for _, id := range []string{"evm.coordinator-upgrade-implementation", "evm.coordinator-upgrade-activate", "fleet.refresh.deploy-batcher"} {
		if !exactVerifiedPlanAction(source, entries, id) {
			return nil, fmt.Errorf("coordinator repair predecessor %s lacks exact original verification", id)
		}
	}
	if err := configureCoordinatorUpgradeNonce(built, observation.reference.Request.Request.Upgrade.DeployerNonce); err != nil {
		return nil, err
	}
	if source.CoordinatorUpgradeBaseline.Schema == "urnetwork-coordinator-upgrade-baseline-v4" {
		if err := configurePrecompileProbeNonce(built, source.CoordinatorUpgradeBaseline.ReplacementPrecompileProbeNonce); err != nil {
			return nil, err
		}
	}
	if err := bindCoordinatorRepairCarryPayloads(built, observation); err != nil {
		return nil, err
	}
	...
	if err := validateCoordinatorUpgradeBaselineRelease(baseline, source.Deployment, built.Manifest, source.CoordinatorUpgrade); err != nil {
		return nil, err
	}
	if err := validateCoordinatorUpgradePayloadBaseline(baseline, source.Deployment, coordinatorRepairBaselinePayloads(built, &observation.reference)); err != nil {
		return nil, err
	}
...
func carryCoordinatorRepairPlan(revised, prior *SetupPlan, observation *coordinatorRepairCarryObservation, entries []JournalEntry) error {
	if observation == nil || observation.source == nil {
		return errors.New("coordinator repair plan carry lacks authenticated source")
	}
	reference := observation.reference
	revised.CoordinatorRepairCarry = &reference
	for index, action := range revised.Actions {
		if !coordinatorRepairOriginalAction(action.ID) && action.ID != "fleet.refresh.deploy-batcher" {
			continue
		}
		source, err := exactPlanActionByID(observation.source, action.ID)
		if err != nil || !exactVerifiedPlanAction(observation.source, entries, action.ID) {
			return stateMismatchError(err, "coordinator repair cannot carry an unverified predecessor")
		}
...
func (e *Executor) authenticateCoordinatorRepairCarry(ctx context.Context) (*coordinatorRepairCarryObservation, error) {
	if e == nil || e.plan == nil || e.plan.CoordinatorRepairCarry == nil || e.deployer == nil || e.journal == nil {
		return nil, errors.New("coordinator repair execution owner is absent")
	}
...
func (e *Executor) verifyCoordinatorRepairCarryAction(ctx context.Context, action Action) error {
	if action.ID != coordinatorRepairCarryActionID || e.plan.CoordinatorRepairCarry == nil || action.Parameters["request_hash"] != e.plan.CoordinatorRepairCarry.Request.Hash || action.Parameters["result_hash"] != e.plan.CoordinatorRepairCarry.Result.Hash || action.Parameters["source_plan_hash"] != e.plan.CoordinatorRepairCarry.Request.Request.PlanHash {
		return errors.New("coordinator repair admission action differs from approved carry")
	}
...
func (e *Executor) verifyCoordinatorRepairOriginalAction(ctx context.Context, action Action, verified JournalEntry, record *ActionPostcondition) error {
	...
	source, err := exactPlanActionByID(observation.source, action.ID)
	if err != nil || !reflect.DeepEqual(action, source) || !observation.source.allowedPlanHashes()[verified.PlanHash] || !actionAcceptsIntent(source, verified.IntentHash) {
		return errors.New("coordinator repair predecessor differs from exact original action")
	}
	if record == nil || record.EVMFinalized.Number >= observation.reference.Result.Result.Activate.BlockNumber || record.IndependentEVMFinalized.Number >= observation.reference.Result.Result.Activate.BlockNumber {
		return errors.New("coordinator repair predecessor verification is not historical")
	}
	copy := *e
