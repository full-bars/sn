...
# Isolate from the network and the real HOME: with real curl/jq on PATH the
# update flow can contact GitHub and even write persistent state under
# $HOME. Stub curl/wget/systemctl to fail and pin HOME to TEMP_DIR; the
# failing network mocks terminate the flow right after "Fetching release
# information" (before any download or persistent write) while preserving
# the argument-parsing assertions below.
mkdir -p "$TEMP_DIR/mock-bin"
for mock_tool in curl wget systemctl; do
    printf '#!/bin/sh\nexit 1\n' > "$TEMP_DIR/mock-bin/$mock_tool"
    chmod +x "$TEMP_DIR/mock-bin/$mock_tool"
done
...
output=$(HOME="$TEMP_DIR" PATH="$TEMP_DIR/mock-bin:$PATH" "$TEMP_DIR/urnet-tools" update 2>&1) || true
...
# Same isolation as Test 3: failing network mocks + pinned HOME guarantee no
# GitHub contact and no writes outside TEMP_DIR, while the flow still
# proceeds past arg parsing into install mode.
output=$(HOME="$TEMP_DIR" PATH="$TEMP_DIR/mock-bin:$PATH" "$TEMP_DIR/Provider_Install_Linux.sh" 2>&1) || true
