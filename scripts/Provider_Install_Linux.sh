...
show_help ()
{
    ...
    echo "  report [<url>|off]       📡  Show or set report URL"
    ...
    echo ""
    echo "Need help? Email support@fullbars.xyz or visit <https://github.com/full-bars/sn>"
}
...
urnet_install_url="${URNET_INSTALL_URL:-https://raw.githubusercontent.com/full-bars/sn/refs/heads/main/scripts/Provider_Install_Linux.sh}"
...
# (-k for curl, --no-check-certificate for wget). Used during TLS TOFU
...
do_install ()
{
    ...
    # (urnet-tools-<os>-<arch>, v3.23.0-fix.28+ / v2026+). Prefer it — digest-verified
...
# of a stale URNETWORK_REPORT_URL an older urnet-tools version may have left behind.
...
do_set ()
{
    ...
    if [ "$key" = "help" ]; then
        ...
        echo "Available keys:"
        echo ""
        echo "  node-name           <string>    Node name reported to the fleet (default: hostname)"
