...
show_help() {
    ...
    echo ""
    echo "https://github.com/full-bars/sn"
}
...
github_api="https://api.github.com/repos/full-bars/sn"
github_raw="https://raw.githubusercontent.com/full-bars/sn/refs/heads/main"
...
do_install() {
    ...
    # the caller's tag directly. There is no separate mirror — downloads
    # always come from the official GitHub release assets.
    if [ -z "$tag" ]; then
        if [ "$version" != "latest" ]; then
            tag="$version"
        else
    ...
    tarball_url="https://github.com/full-bars/sn/releases/download/$tag/urnetwork-provider-$tag.tar.gz"
    mirror_url="$tarball_url"
    pr_info "Downloading %s..." "$tarball_url"

    tmpdir="$(mktemp -d)"
    if ! curl -fsSL "$tarball_url" -o "$tmpdir/provider.tar.gz"; then
        pr_warn "Download failed, retrying..."
    ...
    if [ -n "$tool_digest" ]; then
        if curl -fsSL "https://github.com/full-bars/sn/releases/download/$tag/$tool_asset" -o "$tmpdir/$tool_asset" 2>/dev/null; then
