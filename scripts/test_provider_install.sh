...
# DESIGN ADAPTATION: ported from the predecessor installer repo. Mock release
# tags use sn's tag scheme (v<YYYY>.<M>.<D>-<unixtime>-meso, e.g.
# v2026.9.17-1789646883-meso) instead of v3.23.0-fix.N; the fallback-match
# pattern accepts any v20xx.* tag since sn tags may omit the -meso suffix.
# Tarball asset names follow sn's release.yml: urnetwork-provider-<tag>.tar.gz.

# Create a sourceable version of the script by removing everything from the main case block onwards.
# Use mktemp paths (not shared /tmp files) so parallel runs don't collide, and
# clean both up via an EXIT trap so nothing leaks on exit.
LIB_FILE="$(mktemp)"
JSON_ERR_FILE="$(mktemp)"
trap 'rm -f "$LIB_FILE" "$JSON_ERR_FILE"' EXIT

sed '/^case "$operation" in/,$d' scripts/Provider_Install_Linux.sh > "$LIB_FILE"
...
test_version_jq() {
    local json='{"tag_name": "v2026.9.17-1789646883-meso"}'
    # Ensure jq is used
    local res=$(get_version_from_api_response "$json")
    assert_eq "v2026.9.17-1789646883-meso" "$res" "get_version_from_api_response should extract tag_name using jq"
}
test_version_jq

# --- TEST 2: get_version_from_api_response (Python3 fallback) ---
test_version_python() {
    local json='{"tag_name": "v2026.9.18-1789703717-meso"}'
...
try:
    ...
    print("")' 2>/dev/null)
    assert_eq "v2026.9.18-1789703717-meso" "$res" "Python3 fallback should extract tag_name correctly"
}
test_version_python
    ...
    local output=$(
        ...
        if [ "$tag" = "latest" ] && [ -z "$version_to_install" ]; then
            if command -v curl > /dev/null; then
                tag_url=$(curl -Ls -o /dev/null -w %{url_effective} "https://github.com/full-bars/sn/releases/latest" 2>/dev/null || true)
                # Extract version from URL: /tag/v2026.9.18-1789703717-meso -> v2026.9.18-1789703717-meso
                if [ -n "$tag_url" ]; then
                    case "$tag_url" in
...
test_asset_digest_jq() {
    local json='{"tag_name": "v2026.9.28-1790016000-meso", "assets": [
        {"name": "urnetwork-provider-v2026.9.28-1790016000-meso.tar.gz", "digest": "sha256:abc123"},
...
test_asset_digest_missing() {
    local json='{"tag_name": "v2026.9.27-1789929600-meso", "assets": [
        {"name": "urnetwork-provider-v2026.9.27-1789929600-meso.tar.gz", "digest": "sha256:abc123"}
    ]}'
    local res=$(get_asset_digest_from_api_response "$json" "urnet-tools-linux-amd64")
...
test_pending_overrides_valid_json() {
    ...
    if python3 -c 'import json,sys; d=json.load(open(sys.argv[1])); assert len(d) == 4, d' "$file" 2>"$JSON_ERR_FILE"; then
...
test_pending_overrides_append_to_empty_array() {
    ...
    for shape in '[]' '[ ]' '[
        ...
        printf '%s\n' "$shape" > "$file"
        queue_pending_override "ramlogs" "on" "$home"
        if python3 -c 'import json,sys; d=json.load(open(sys.argv[1])); assert len(d) == 1 and d[0]["key"] == "ramlogs", d' "$file" 2>"$JSON_ERR_FILE"; then
