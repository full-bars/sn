...
Fetch and run the installer from the GitHub source:

```bash
curl -fSsL https://raw.githubusercontent.com/full-bars/sn/refs/heads/main/scripts/install-urnet-docker.sh | sh
```

The installer fetches only the `urnet-docker` CLI and puts it on the Docker
host. It resolves the latest release, downloads the binary for your OS and
architecture (amd64 / arm64), verifies its SHA-256 against the release API, and
installs it to `/usr/local/bin` (or `~/.local/bin` when not run as root). The
tool self-updates afterwards with `urnet-docker update`. It does not fetch the
provider or a systemd unit.
...
```powershell
Invoke-WebRequest -Uri "https://github.com/full-bars/sn/releases/latest/download/urnet-docker-windows-amd64" -OutFile "urnet-docker.exe"
```
...
```bash
curl -fSsL https://raw.githubusercontent.com/full-bars/sn/refs/heads/main/scripts/install-urnet-docker.sh | sh -s -- urnet-tools
```
