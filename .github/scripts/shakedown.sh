...
curl -fSsL https://raw.githubusercontent.com/full-bars/sn/refs/heads/main/scripts/Provider_Install_Linux.sh -o /tmp/install.sh
...
if timeout 10 curl -fsS -o /dev/null https://raw.githubusercontent.com/full-bars/sn/main/README.md 2>/dev/null; then
  ok "github reachable"
else
...
curl -fSsL https://raw.githubusercontent.com/full-bars/sn/refs/heads/main/scripts/install-urnet-docker.sh -o /tmp/install-docker.sh
...
run_check "image pulled" timeout 300 docker pull "ghcr.io/full-bars/sn:${EXPECTED_VERSION}" 2>&1
# MUST-FIX 6 (docker): pass the cap into the container via env var.
docker run -d --name urnetwork-test -v /tmp/docker-state:/root/.urnetwork -e PROXY_URL_MAX=200 -e BUILD=jwt "ghcr.io/full-bars/sn:${EXPECTED_VERSION}" >/dev/null 2>&1
sleep 8
